---
title: <center><font size="6"><b>Variation in vocal behavior under varying stress levels V01</b></font></center>
...
sub_agg <- agg_dat[, c("baseline.CORT", "stress.CORT", "call.count", "acoustic.overlap", "acoustic.overlap.to.group", "weight", "breath.rate", "foxp2")]



names(sub_agg) <- c("baseline\nCORT", "stress\nCORT", "vocal\noutput", "acoustic\noverlap\nto self", "acoustic\noverlap\nto group", "weight", "breath\nrate", "FOXP2")
cm <- cor(sub_agg, use = "pairwise.complete.obs")
...
responses <- c("duration", "meanfreq", "sd", "freq.median", "freq.IQR", "time.IQR", "skew",  "kurt", "sp.ent", "time.ent", "entropy", "meandom", "mindom", "maxdom", "dfrange",  "modindx", "meanpeakf")
...
acous_models <- lapply(1:length(formulas), function(x) {
    ...
    mod <- brm(formula = formulas[x],
        iter = 20000, silent = 2, data = sub_agg_dat, control = list(adapt_delta = 0.9),
        chains = 4,
        prior = c(prior(normal(0, 5), "b"), prior(normal(0, 10), "Intercept"),
    prior(student_t(3, 0, 10), "sd"), prior(student_t(3, 0, 10), "sigma")),
            file = paste0("./data/processed/", responses[x], "_model"),
