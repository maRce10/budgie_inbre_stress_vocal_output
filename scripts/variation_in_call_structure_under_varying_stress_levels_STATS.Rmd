...
gg_acoustic.overlap.group <- ggplot(data = agg_behav, aes(x = week, y = acoustic.overlap.to.group, fill = treatment)) + 
    ...
    scale_fill_viridis_d(begin = 0.1, end = 0.9) + 
    facet_wrap(~treatment, ncol = 3, scale = "fixed") +
    labs(y = "Acoustic overlap to group", x = "Week") + 
    theme_classic(base_size = bs) + 
    theme(legend.position = "none")
...
for (x in 1:length(behav_models)) {
    ...
    ggl[[length(ggl) + 1]] <- summary_brm_model(behav_models[[x]], gsub.pattern = "b_treatment|b_", gsub.replacement = "")
    }
...
cowplot::plot_grid(gg_call.count + theme_classic(base_size = 10) + 
    theme(legend.position = "none"), ggl[[grep("count", names(ggl))]])

cowplot::plot_grid(gg_acoustic.area + theme_classic(base_size = 10) + 
    theme(legend.position = "none"), ggl[[grep("area", names(ggl))]])

cowplot::plot_grid(gg_acoustic.overlap + theme_classic(base_size = 10) + 
    theme(legend.position = "none"), ggl[[grep("overlap ~", names(ggl))]])

cowplot::plot_grid(gg_acoustic.overlap.group + theme_classic(base_size = 10) + 
    theme(legend.position = "none"), ggl[[grep("group", names(ggl))]])
